...
func ToGoogleMessages(messages []types.Message) []map[string]interface{} {
	...
	for _, msg := range messages {
		...
		for _, part := range msg.Content {
			...
			case types.CustomContent:
				// CustomContent in assistant messages may carry Google-specific
				// provider options. Forward the google-keyed options verbatim if
				// present; otherwise skip.
			...
			case types.ReasoningFileContent:
				// Reasoning files generated by the model are not re-sent to Google.
			}
		}
