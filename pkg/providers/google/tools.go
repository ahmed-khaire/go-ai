...
// Requires Gemini 2.0 or newer. Only available on Vertex AI.
...
// buildGoogleNativeToolEntry converts a provider Tool to the Google API native tool
// entry format. Returns nil when the ProviderID is unrecognized.
func buildGoogleNativeToolEntry(t types.Tool) map[string]interface{} {
	args := t.ProviderArgs
	switch t.ProviderID {
